...
function sprintf_format(parse_tree: string, argv: any[]) {
  ...
  for (i = 0; i < tree_length; i++) {
    if (typeof parse_tree[i] === "string") {
      ...
      let ph: any = parse_tree[i]; // convenience purposes only
      if (ph.keys) {
        // keyword argument
        arg = argv[cursor];
...
interface SprintFCache {
  [key: string]: any;
}

let sprintf_cache: SprintFCache = {};
  ...
  while (_fmt) {
    if ((match = re.text.exec(_fmt)) !== null) {
      ...
      if (match[2]) {
        ...
        let field_match: any = re.key.exec(replacement_field);
        if (field_match !== null) {
...
export function vsprintf(text: string, argv: any[]) {
  return sprintf(text, ...argv);
}

export default function sprintf(text: string, ...args: any[]) {
  return sprintf_format(sprintf_parse(text), args);
}
